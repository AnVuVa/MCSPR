"""Train HisToGene baseline (no MCSPR) -- all LOPCV folds.

Graph-based ViT operating on whole slides. Uses batch_size=1 with
whole_slide_collate_fn; routes through universal_trainer._graph_based_step.

Usage:
    python src/experiments/run_histogene.py --config configs/her2st.yaml [--dry_run]

Saves results to: results/histogene/{dataset}/
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train HisToGene baseline")
    ...
    for fold_idx in fold_range:
        ...
        if args.dry_run and fold_idx == fold_range[0]:
            ...
            if isinstance(batch, list) and batch:
                for k, v in batch[0].items():
                    if isinstance(v, torch.Tensor):
                        print(f"  slide[0].{k}: {tuple(v.shape)} {v.dtype}")
                    else:
                        print(f"  slide[0].{k}: {type(v).__name__}")
            ...
            print("Dry run complete (HisToGene).")
            return
    ...
    if all_fold_metrics:
        summary = _aggregate(all_fold_metrics, "histogene", dataset)
        with open(output_dir / "summary.json", "w") as f:
            json.dump(summary, f, indent=2)
        print(f"\n{'=' * 50}")
        print(f"FINAL RESULTS -- HisToGene on {dataset}")
        print(f"{'=' * 50}")
