"""Canonical per-slide evaluation for HydraSTNet (PCC(M), PCC(H), MSE, MAE).

Reuses the exact same `_per_slide_metrics` calculator as canonical_eval.py
(STNet/HisToGene/TRIPLEX/MERGE). Reassembles each fold's K head outputs into
a single (B, 300) tensor in registry-canonical column order, groups by
sample_idx, and feeds the resulting per-slide arrays into the canonical
metric.

Usage:
  python scripts/canonical_eval_stnet_hydra.py \
      --config configs/her2st.yaml \
      --input_dir results/baselines/stnet_hydra \
      [--folds 0,1,2,3]
"""
...
from src.data.loaders import build_stnet_hydra_loaders  # noqa: E402
from src.models.stnet_hydra import HydraSTNet  # noqa: E402
from src.training.hydra_helpers import (  # noqa: E402
    load_registry,
...
def _eval_one_fold(
    ...
    idx_list = verify_modules(
        val_loader.dataset, registry, fold_idx=fold,
    )
...
def main() -> None:
    ...
    print(f"=== Canonical eval: stnet_hydra ===")
